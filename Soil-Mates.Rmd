...
library(vegan)
library(tidyverse)
library(ggpubr) # used to adds stats in plots 
```
...
####
```{r}
ggplot(EnM, aes(x=Disturbance_History, y=Div)) +
    geom_boxplot(aes(colour=I("black"), fill=Disturbance_History)) +  
    labs(y="Alpha Diversity", 
         x="Depth of sample",
         fill="Disturbance History") + 
  #scale_x_discrete(labels=c("0-10cm", "20-30cm")) +
  geom_hline(yintercept = mean(EnM$Disturbance_History), color="blue") +
        theme_classic()
```
...
```{r}
ggplot(EnM, aes(x=reorder(Disturbance_History,-TSD), y=Div)) +
    geom_boxplot(aes(colour=I("black"), fill=Disturbance_History)) +  
    labs(y="Alpha Diversity", 
...
ggplot(EnM, aes(x=No.Fires, y=Div)) +
...
```{r}
EnM2 <- EnM
EnM2$TSD <- as.character(EnM2$TSD)
ggplot(EnM2, aes(x=TSD, y=Div)) +
...
ggplot(EnM, aes(x=Clearcut, y=Div)) +
...
ggplot(EnM, aes(x=Salvage, y=Div)) +
    geom_boxplot(aes(colour=I("black"), fill= Salvage)) +  
    labs(y="Alpha Diversity", fill="Number of Fires") + 
        theme_classic() +
  stat_compare_means(method = "t.test", label.x= 1.5)
```
