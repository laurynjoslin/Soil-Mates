...
```{r}
qplot(x=EnQ1$No.Fires, y=EnQ1$Div)
```

```{r}
qplot(No.Fires, Microbial_diversity, data=EnQ1) 
```
...
EnM <- En
EnM <- EnM %>%
  select(No.Fires, Clearcut, Salvage, TSD) %>%
  transmute(No.Fires+Clearcut+Salvage+TSD)
EnM
unique(EnM)
```
