...
library(tidyverse)
```
#### Importing all at one - may be a pain 
```{r}
...
#### Add proper names to a new column for Destructive History in a New environemental table - Needing fixing possibly a loop or something
```{r}
EnM <- EnQ1
EnM$Destructive_History[EnM$No.Fires== 0 & EnM$Clearcut == 0 & EnM$Salvage == 0 & EnM$TSD == 168] <- "1850F"
...
```{r}
ggplot(EnM, aes(x=Destructive_History, y=Div)) +
