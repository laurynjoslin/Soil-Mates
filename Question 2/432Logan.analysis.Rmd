...
```{r}
a <- ggplot(en)+
  geom_boxplot(aes(x = no_fires, y = Nitrate.Nitrogen, fill = clearcut), outlier.shape = NA, alpha=0.25)+
...
```{r}
h <- ggplot(en)+
  geom_boxplot(aes(x = no_fires, y = Nitrate.Nitrogen, fill = Salvage), outlier.shape = NA, alpha=0.25)+
