...
Making this a function---> STOPPED HERE!!!!
...
```{r}
NMDSdat_arc = NMDSdat_make(arc_nonan_bray[[1]])
NMDSdat_bac = NMDSdat_make(bac_nonan_bray[[1]])
NMDSdat_fun = NMDSdat_make(fun_nonan_bray[[1]])

NMDSdat_all = NMDSdat_make(all_nonan_bray[[1]])
```
...
for (i in features) {
  print(qplot(x=NMDS1,y=NMDS2,color=as.factor(get(i)), alpha= I(0.6), data=NMDSdat_all)+
  stat_ellipse() + 
  theme_bw()+
   guides(color=guide_legend(title=i)) )
...
# Truee cont feeatures--> to add ellippse, wee need a low vs high
cont = c('TSD', 'Ammonium.Nitrogen', 'Nitrate.Nitrogen', 'Phosphorus.Colwell', 'Potassium.Colwell', 'Organic.Carbon', 'DTPA.Copper', 'DTPA.Iron','DTPA.Manganese',  'Exc.Magnesium','Slope', 'Elevation', 'pH.CaCl2.')

for (i in cont) {
  print(qplot(x=NMDS1,y=NMDS2,color=get(i), alpha= I(0.6), data=NMDSdat_all)+
  stat_ellipse() + 
  theme_bw()+
   guides(color=guide_legend(title=i)) )
...
Lookin within a depth condition

```{r}
ggplot(data = NMDSdat_bac[NMDSdat_bac$depth =='10',], aes(NMDS1, NMDS2, color=as.factor(Clearcut))) + 
  geom_point() +
    stat_ellipse() + 
  theme_bw()

ggplot(data = NMDSdat_bac[NMDSdat_bac$depth =='20',], aes(NMDS1, NMDS2, color=as.factor(Clearcut))) + 
  geom_point() +
    stat_ellipse() + 
  theme_bw()
...
# focus on this one.. ddistinct clustering
# maybe DTPA.Iron
ggplot(data = NMDSdat_bac[NMDSdat_bac$depth =='10',], aes(NMDS1, NMDS2 )) + 
  geom_point(aes(color=DTPA.Iron )) +
    stat_ellipse(aes( group =No.Fires)) + 
  theme_bw()

ggplot(data = NMDSdat_bac[NMDSdat_bac$depth =='10',], aes(NMDS1, NMDS2 )) + 
  geom_point(aes(color=as.factor(No.Fires ))) +
    stat_ellipse(aes( group =as.factor(No.Fires), color=as.factor(No.Fires ))) + 
  theme_bw()

# mabee pH.CaCl2.??
ggplot(data = NMDSdat_bac[NMDSdat_bac$depth =='10',], aes(NMDS1, NMDS2 )) + 
  geom_point(aes(color=pH.CaCl2. )) +
    stat_ellipse(aes( group =No.Fires)) + 
  theme_bw()
...
# maybe ph??
ggplot(data = NMDSdat_bac[NMDSdat_bac$depth =='20',], aes(NMDS1, NMDS2 )) + 
  geom_point(aes(color=pH.CaCl2. )) +
    stat_ellipse(aes( group =No.Fires)) + 
  theme_bw()
...
```{r}
qplot(x=NMDS1,y=NMDS2,color=Organic.Carbon, alpha= I(0.6), data=NMDSdat_bac[NMDSdat_bac$depth =='10',])+
  theme_bw()


qplot(x=NMDS1,y=NMDS2,color=as.factor(Salvage), alpha= I(0.6), data=NMDSdat_bac[NMDSdat_bac$depth =='20',])+
  theme_bw()
```
